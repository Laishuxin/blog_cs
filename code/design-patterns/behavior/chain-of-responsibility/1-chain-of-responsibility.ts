...
abstract class Handler {
  ...
  public setNextChain(handler: Handler) {
...
class Handler1 extends Handler {
  protected canHandleRequest(request: Request): boolean {
    console.log(
      `handler1 can handle? ${request.getType() === RequestType.HANDLER1}`,
    )
    return request.getType() === RequestType.HANDLER1
  }
  protected processRequest(request: Request): void {
    console.log(
      `handle1: ${request.getDescription()}, type: ${request.getType()}`,
    )
  }
}
class Handler2 extends Handler {
  protected canHandleRequest(request: Request): boolean {
    console.log(
      `handler2 can handle? ${request.getType() === RequestType.HANDLER2}`,
    )
    return request.getType() === RequestType.HANDLER2
  }
  protected processRequest(request: Request): void {
    console.log(
      `handler2: ${request.getDescription()}, type: ${request.getType()}`,
    )
  }
}
